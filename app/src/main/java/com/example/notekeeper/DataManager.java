...
public class DataManager {
    ...
    private static void loadNotesFromDatabase(Cursor cursor) {
        int courseIdColumnIndex = cursor.getColumnIndex(NoteInfoEntry.COLUMN_COURSE_ID);
        int noteTitleColumnIndex = cursor.getColumnIndex(NoteInfoEntry.COLUMN_NOTE_TITLE);
        int noteTextColumnIndex = cursor.getColumnIndex(NoteInfoEntry.COLUMN_NOTE_TEXT);
        int idColumnIndex = cursor.getColumnIndex(NoteInfoEntry._ID);
    ...
    private static void loadCoursesFromDatabase(Cursor cursor) {
        ...
        int courseTitleColumnIndex = cursor.getColumnIndex(CourseInfoEntry.COLUMN_COURSE_TITLE);
        DataManager dm = getInstance();
        dm.mCourses.clear();

        while (cursor.moveToNext()) {
            String courseId = cursor.getString(courseIdColumnIndex);
            String courseTitle = cursor.getString(courseTitleColumnIndex);
